...
namespace Social
{
	public class SocialPlayerController : EntityComponent<SocialPlayer> 
	{
		...
		public void Simulate( IClient cl )
		{
			...
			if ( groundEntity.IsValid() )
			{
				...
				Entity.Velocity = Accelerate( Entity.Velocity, moveVector.Normal, moveVector.Length, 200f * (Input.Down( InputButton.Run ) ? 2.5f : 1f), 7.5f );
			...
			if ( moveHelper.TryMoveWithStep(Time.Delta, StepSize) > 0 )
			{
				if ( Grounded )
				{
					moveHelper.Position = StayOnGround( moveHelper.Position );
				}
		...
		static Vector3 ApplyFriction( Vector3 input, float frictionAmount )
		{
		...
		static Vector3 Accelerate( Vector3 input, Vector3 wishdir, float wishspeed, float speedLimit, float acceleration )
		{
